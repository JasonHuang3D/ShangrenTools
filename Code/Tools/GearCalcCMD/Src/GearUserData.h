...
DEFINE_FLAG_ENUM_OPERATORS(PropertyMask);
...
struct XianRenPropBuff
{
    ...
    {
        if constexpr ((PropMask & PropertyMask::Li) != PropertyMask::None)
        {
            li_add += right.li_add;
            li_percent += right.li_percent;
        }
        if constexpr ((PropMask & PropertyMask::Nian) != PropertyMask::None)
        {
            nian_add += right.nian_add;
            nian_percent += right.nian_percent;
        }

        if constexpr ((PropMask & PropertyMask::Fu) != PropertyMask::None)
        {
            fu_add += right.fu_add;
...
struct XianRenProp
{
    ...
    void MultiplyBuff(const XianRenPropBuff& buff)
    {
        if constexpr ((PropMask & PropertyMask::Li) != PropertyMask::None)
        {
            li += li * buff.li_percent * 0.01;
        }
        if constexpr ((PropMask & PropertyMask::Nian) != PropertyMask::None)
        {
            nian += nian * buff.nian_percent * 0.01;
        }
        if constexpr ((PropMask & PropertyMask::Fu) != PropertyMask::None)
        {
            fu += fu * buff.fu_percent * 0.01;
        }
    }
    template<PropertyMask PropMask = PropertyMask::All>
...
public:
    ...
    std::unordered_map<std::string, const GearData* const> const GetLoopUpGears(){ return m_loopUpGears; }
...
public:
    ...
    const std::vector<const XianRenData*>& GetCalcXianRenDataVec() const { return m_calcXianRenDataVec; }
